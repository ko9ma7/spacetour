param(
  [ValidateSet('', 'setup', 'publish', 'studio', 'import', 'status', 'backup', 'metadata')]
  [string]$Action = ''
)
...
$ContentRoot = Join-Path $Root 'content\projects'
$BackupRoot = Join-Path $Root 'backups'
$ConfigFile = Join-Path $Root '.spacetour.local.json'
$ManagerVersion = '2.3.0'
...
function Generate-ProjectMetadata {
  Ensure-Tools
  Write-Step 'Generating README and GitHub repository metadata'
  Push-Location $Root
  try {
  ...
  Write-Ok 'README / About / Topics metadata generated'
}

function Sync-GitHubMetadata {
  param([string]$Owner, [string]$Repo)

  $metaFile = Join-Path $Root '.github\repository-metadata.json'
  if (-not (Test-Path $metaFile)) { Generate-ProjectMetadata }
  $meta = Get-Content -LiteralPath $metaFile -Raw | ConvertFrom-Json
  $description = [string]$meta.description
  $siteUrl = Get-SiteUrl $Owner $Repo

  Write-Step 'Synchronizing GitHub About / Website / Topics'

  $previous = $ErrorActionPreference
  try {
    $ErrorActionPreference = 'Continue'
    & gh api --method PATCH "repos/$Owner/$Repo" -f "description=$description" -f "homepage=$siteUrl" 1>$null
    if ($LASTEXITCODE -ne 0) { throw 'GitHub About/Website update failed.' }

    $topics = @($meta.topics | ForEach-Object { [string]$_ } | Where-Object { $_ })
    $topicsPayload = @{ names = $topics } | ConvertTo-Json -Compress
    $temp = Join-Path ([System.IO.Path]::GetTempPath()) ("spacetour-topics-{0}.json" -f ([guid]::NewGuid().ToString('N')))
    try {
      [System.IO.File]::WriteAllText($temp, $topicsPayload, [System.Text.UTF8Encoding]::new($false))
      & gh api --method PUT -H 'Accept: application/vnd.github+json' "repos/$Owner/$Repo/topics" --input $temp 1>$null
      if ($LASTEXITCODE -ne 0) { throw 'GitHub Topics update failed.' }
    } finally {
  ...
  Write-Ok 'GitHub repository metadata synchronized'
  Write-Host "About   : $description" -ForegroundColor DarkGray
  Write-Host "Website : $siteUrl" -ForegroundColor DarkGray
  Write-Host "Topics  : $((@($meta.topics) -join ', '))" -ForegroundColor DarkGray
}
...
function Setup-Project {
  ...
  Sync-GitHubMetadata -Owner $origin.Owner -Repo $origin.Repo
...
function Publish-Changes {
  ...
  if ($LASTEXITCODE -eq 0) {
    ...
    & gh workflow run deploy.yml --ref main 2>$null
    if ($LASTEXITCODE -eq 0) { Write-Ok 'Immediate GitHub Pages deployment requested.' } else { Write-Warn 'Immediate deployment request failed. The 30-minute scheduled deployment will retry.' }
  }
...
function Open-Studio {
  ...
  Ensure-Tools
  $url = 'http://127.0.0.1:5173/studio/#/admin'
  $alive = $false
  try {
  ...
  Write-Host 'Local-only Studio opened. It is never deployed to GitHub Pages.' -ForegroundColor DarkGray
}
...
function Backup-Content {
  ...
  if (-not (Test-Path $ContentRoot)) { throw 'The content\projects folder does not exist.' }
...
function Update-RepositoryMetadata {
  Write-Title 'README + GITHUB ABOUT + TOPICS'
  Ensure-Tools
  [void](Ensure-GitHubLogin)
  Generate-ProjectMetadata
  $origin = Get-OriginInfo
  if (-not $origin) { throw 'No GitHub origin is connected. Run FIRST-SETUP.cmd first.' }
  Sync-GitHubMetadata -Owner $origin.Owner -Repo $origin.Repo
  [void](Ensure-LocalCommit 'Update SpaceTour repository metadata')
  Invoke-Native git -C $Root push origin main
  Write-Ok 'README and GitHub metadata update complete.'
}
...
function Run-RequestedAction {
  switch ($Action) {
    ...
    'metadata' { Update-RepositoryMetadata; return $true }
...
try {
  ...
  while ($true) {
    ...
    Write-Host '  8. Open content\projects folder' -ForegroundColor White
    Write-Host '  9. Regenerate README + sync GitHub About/Topics' -ForegroundColor White
    ...
    try {
      switch ($choice) {
        ...
        '9' { Update-RepositoryMetadata; Wait-Key }
        '0' { break }
        default { Write-Warn 'Choose a number from 0 to 9.'; Start-Sleep -Seconds 1 }
      }
    } catch {
