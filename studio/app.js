...
import { makeZip } from '/studio/lib/zip.js';
import { imageDimensions, toWebp } from '/studio/lib/image.js';
import { mountMap, mountKakaoRoadview, searchLocations, externalMapUrl } from '/lib/maps.js';

const app = document.querySelector('#app');
const pageBase = new URL('/', location.origin);
...
const normalizeDeg = v => ((Number(v||0)%360)+360)%360;
const directionLabel = v => ['위쪽','오른쪽 위','오른쪽','오른쪽 아래','아래쪽','왼쪽 아래','왼쪽','왼쪽 위'][Math.round(normalizeDeg(v)/45)%8];
...
function topbar(){return `<header class="topbar"><div class="container topbar-inner">${brand()}<nav class="topnav"><a class="nav-link" href="/#/" target="_blank">고객 화면 미리보기</a><a class="nav-link" href="#/admin">Studio</a><button class="icon-btn theme-btn" title="테마 전환" aria-label="테마 전환">◐</button></nav></div></header>`}
...
class Studio {
  constructor(){
    this.meta={title:'',slug:'',subtitle:'APARTMENT · INTERACTIVE TOUR',status:'공급중',address:'',placeName:'',lat:'',lng:'',priceLabel:'',areaLabel:'',orientation:'',description:'',contactName:'',contactPhone:'',contactMessage:'',planNorthDeg:'',...this.readDraft()};
  ...
  planPanel(){
    const north=this.meta.planNorthDeg===''?'':normalizeDeg(this.meta.planNorthDeg);
    const northOptions=[['','미설정'],[0,'위쪽 ↑'],[45,'오른쪽 위 ↗'],[90,'오른쪽 →'],[135,'오른쪽 아래 ↘'],[180,'아래쪽 ↓'],[225,'왼쪽 아래 ↙'],[270,'왼쪽 ←'],[315,'왼쪽 위 ↖']];
    return `<div class="panel">${this.head('04','평면도 · 촬영 위치 · 보는 방향','도면에서 사진을 찍은 위치와 카메라가 바라보는 방향을 함께 등록합니다. 360°는 고객이 화면을 돌릴 때 평면도의 시야 표시도 같이 회전합니다.')} ${!this.planPreview?`<label class="plan-upload"><b>＋ 평면도 이미지 선택</b><span>JPG · PNG · WEBP</span><input id="plan-input" type="file" accept="image/*"></label>`:`<div class="north-setting"><div><b>도면의 북쪽 방향 <span>선택사항</span></b><small>설정하면 고객에게 “실제 동/서/남/북 방향”까지 표시합니다. 모르면 미설정으로 두세요.</small></div><select id="plan-north">${northOptions.map(([v,l])=>`<option value="${v}" ${String(v)===String(north)?'selected':''}>${l}</option>`).join('')}</select></div><div class="plan-layout"><div><div class="plan-tip">① 도면에서 촬영 위치 클릭 → ② 연결할 사진/360° 선택 → ③ 카메라가 바라본 방향 지정</div><div class="plan-edit-stage direction-plan" id="plan-stage"><img src="${esc(this.planPreview)}" alt="편집 중인 평면도">${this.hotspots.map((h,i)=>{const deg=normalizeDeg(h.directionDeg);return `<button class="hotspot direction-hotspot active" style="left:${h.x*100}%;top:${h.y*100}%;--heading:${deg}deg;--cone-scale:.85" title="${esc(h.label)} · ${directionLabel(deg)}"><span class="direction-indicator" aria-hidden="true"><i></i></span><span class="hotspot-core">${i+1}</span></button>`}).join('')}</div><div class="plan-admin-help"><b>표시 읽는 법</b><span>● 원 = 촬영한 위치</span><span>▲ 주황 시야 = 사진/카메라가 보는 방향</span>${north!==''?`<span>⌖ 북쪽 = ${directionLabel(north)}</span>`:''}</div><label class="replace-plan">평면도 교체<input id="plan-input" type="file" accept="image/*"></label></div><div class="hotspot-list"><h3>공간 연결 ${this.hotspots.length}</h3>${this.hotspots.length?this.hotspots.map((h,i)=>{const deg=normalizeDeg(h.directionDeg);return `<div class="hot-row direction-row"><span class="hot-num">${i+1}</span><div class="hot-fields"><input class="mini-input" data-hot-label="${h.id}" value="${esc(h.label)}" placeholder="예: 거실"><select data-hot-target="${h.id}"><option value="">사진/360° 선택</option>${this.media.map(m=>`<option value="${m.id}" ${m.id===h.targetMediaId?'selected':''}>${esc(m.title)} · ${m.type==='panorama'?'360°':m.type}</option>`).join('')}</select><label class="direction-control"><span>보는 방향 <b data-direction-output="${h.id}">${directionLabel(deg)} · ${Math.round(deg)}°</b></span><input type="range" min="0" max="355" step="5" value="${Math.round(deg/5)*5%360}" data-hot-direction="${h.id}"><div class="direction-presets">${[[0,'↑'],[45,'↗'],[90,'→'],[135,'↘'],[180,'↓'],[225,'↙'],[270,'←'],[315,'↖']].map(([d,a])=>`<button type="button" data-hot-dirpreset="${h.id}" data-deg="${d}" class="${Math.abs(normalizeDeg(deg)-d)<3?'active':''}" title="${directionLabel(d)}">${a}</button>`).join('')}</div></label></div><button class="del" data-hot-del="${h.id}">×</button></div>`}).join(''):'<div class="empty-editor" style="min-height:120px"><span>도면에서 실제 사진/360°를 촬영한 위치를 클릭하세요.</span></div>'}</div></div>`}${this.foot('publish','미리보기 / 게시')}</div>`
  }
  publishPanel(){
  ...
  bindPlan(){
    ...
    const north=$('#plan-north'); if(north)north.onchange=e=>{this.meta.planNorthDeg=e.target.value===''?'':Number(e.target.value);this.saveDraft();this.render()};
    const st=$('#plan-stage'); if(st)st.onclick=e=>{if(e.target.closest('button'))return;const r=st.getBoundingClientRect();this.hotspots.push({id:uid(),x:(e.clientX-r.left)/r.width,y:(e.clientY-r.top)/r.height,label:`공간 ${this.hotspots.length+1}`,targetMediaId:this.media[0]?.id||'',directionDeg:0});this.render()};
    ...
    const setDirection=(h,deg)=>{h.directionDeg=normalizeDeg(deg);const out=$(`[data-direction-output="${h.id}"]`);if(out)out.textContent=`${directionLabel(h.directionDeg)} · ${Math.round(h.directionDeg)}°`;const idx=this.hotspots.indexOf(h);const marker=$$('#plan-stage .direction-hotspot')[idx];if(marker)marker.style.setProperty('--heading',`${h.directionDeg}deg`);this.saveDraft()};
    $$('[data-hot-direction]').forEach(r=>r.oninput=e=>{const h=this.hotspots.find(x=>x.id===e.target.dataset.hotDirection);if(h)setDirection(h,Number(e.target.value)||0)});
    $$('[data-hot-dirpreset]').forEach(b=>b.onclick=e=>{const h=this.hotspots.find(x=>x.id===e.currentTarget.dataset.hotDirpreset);if(!h)return;setDirection(h,Number(e.currentTarget.dataset.deg)||0);this.render()});
  ...
  async loadExisting(slug){
    ...
    try{
      ...
      this.meta={title:p.title||'',slug:p.slug||slug,subtitle:p.subtitle||'',status:p.status||'',address:p.address||p.location?.address||'',placeName:p.location?.placeName||'',lat:p.location?.lat??'',lng:p.location?.lng??'',priceLabel:p.priceLabel||'',areaLabel:p.areaLabel||'',orientation:p.orientation||'',description:p.description||'',contactName:p.contact?.name||'',contactPhone:p.contact?.phone||'',contactMessage:p.contact?.message||'',planNorthDeg:p.floorPlan?.northDeg??''};
      this.media=[];
  ...
  async buildPack(){
    ...
    let floorPlan;if(this.planFile){files.set('floorplan.webp',await toWebp(this.planFile,2200,.9));floorPlan={image:'floorplan.webp',alt:`${this.meta.title} 평면도`,northDeg:this.meta.planNorthDeg===''?undefined:normalizeDeg(this.meta.planNorthDeg),hotspots:this.hotspots.map(h=>({...h,directionDeg:normalizeDeg(h.directionDeg),targetMediaId:h.targetMediaId||undefined}))}}
  ...
  async publishNow(){
    ...
    try{
      ...
      payload.deploy='now'; const r=await fetch('/api/publish',{method:'POST',headers:{'Content-Type':'application/json','X-SpaceTour-Token':this.publisher?.token||''},body:JSON.stringify(payload)}); const j=await r.json(); if(!r.ok||!j.ok)throw Error(j.error||'게시 실패');
