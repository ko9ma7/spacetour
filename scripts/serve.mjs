...
execFileSync(process.execPath, ['scripts/build.mjs'], { stdio: 'inherit', cwd });
const root = path.resolve('dist');
const studioRoot = path.resolve('studio');
...
function getSiteUrl() {
  ...
  if (!p) return '';
  return p.repo.toLowerCase() === `${p.owner}.github.io`.toLowerCase() ? `https://${p.owner}.github.io/` : `https://${p.owner}.github.io/${p.repo}/`;
}
...
async function readJsonBody(req, max = 350 * 1024 * 1024) {
  if (!String(req.headers['content-type'] || '').toLowerCase().startsWith('application/json')) throw new Error('JSON 요청만 허용됩니다.');
...
function assertLocalWrite(req) {
  const token = String(req.headers['x-spacetour-token'] || '');
  if (!token || token !== sessionToken) throw new Error('로컬 Studio 세션이 올바르지 않습니다. Publisher를 다시 실행해주세요.');
  const origin = String(req.headers.origin || '');
  if (origin && origin !== `http://${host}:${port}` && origin !== `http://localhost:${port}`) throw new Error('로컬 Studio에서만 저장할 수 있습니다.');
}
...
function dispatchDeploy() {
  const r = run('gh', ['workflow','run','deploy.yml','--ref','main'], { allowFail:true });
  if (r.status === 0) return { dispatched:true, message:'GitHub Pages 즉시 배포를 요청했습니다.' };
  return { dispatched:false, message:'파일은 GitHub에 저장됐지만 즉시 배포 요청은 실패했습니다. 30분 자동 배포가 처리합니다.' };
}
function gitPublish(message, { deploy = false } = {}) {
  if (!hasGitRepo()) throw new Error('Git 저장소가 연결되지 않았습니다. REPAIR-NOW를 한 번 실행해주세요.');
  const origin = getOrigin();
  if (!origin) throw new Error('GitHub origin이 연결되지 않았습니다. REPAIR-NOW를 한 번 실행해주세요.');
  git(['add','content/projects','config/site-config.json'], { capture:true });
  let committed = false;
  if (stagedChanges()) {
  ...
  if (push.status !== 0) {
    ...
    if (/fetch first|non-fast-forward|rejected/i.test(text)) throw new Error('GitHub에 PC보다 새로운 변경이 있습니다. REPAIR-NOW.cmd를 한 번 실행한 뒤 다시 게시해주세요.');
    throw new Error(text.trim() || 'GitHub push에 실패했습니다.');
  }
  const deployment = deploy ? dispatchDeploy() : { dispatched:false, message:'다음 30분 자동 배포에서 반영됩니다.' };
  return { committed, origin, siteUrl:getSiteUrl(), ...deployment };
}
...
async function saveSiteConfig(body) {
  ...
  validateBuild();
  return { config:cfg, ...gitPublish('Update SpaceTour map settings', { deploy:true }) };
}
...
async function publishProject(body) {
  ...
  validateBuild();
  return { slug, ...gitPublish(`Publish property: ${slug}`, { deploy: body.deploy === 'now' }) };
}

async function apiHandler(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/publisher-status') {
    const origin = getOrigin();
    return sendJson(res, 200, { ok:true, localPublisher:true, token:sessionToken, git:hasGitRepo(), origin, siteUrl:getSiteUrl(), branch:git(['branch','--show-current'], { allowFail:true }).stdout?.trim() || '' });
  }
  if (req.method === 'POST' && url.pathname === '/api/settings') {
    try { assertLocalWrite(req); return sendJson(res, 200, { ok:true, ...(await saveSiteConfig(await readJsonBody(req, 2 * 1024 * 1024))) }); }
    catch (e) { return sendJson(res, 500, { ok:false, error:e.message || String(e) }); }
  }
  if (req.method === 'POST' && url.pathname === '/api/publish') {
    try { assertLocalWrite(req); return sendJson(res, 200, { ok:true, ...(await publishProject(await readJsonBody(req))) }); }
    catch (e) { return sendJson(res, 500, { ok:false, error:e.message || String(e) }); }
  }
  return false;
}
...
http.createServer(async (req,res)=>{
  try {
    ...
    if (requestUrl.pathname === '/studio' || requestUrl.pathname === '/studio/') return await serveFile(res, studioRoot, 'index.html');
    if (requestUrl.pathname.startsWith('/studio/')) return await serveFile(res, studioRoot, decodeURIComponent(requestUrl.pathname.slice('/studio/'.length)) || 'index.html');
    const rel = decodeURIComponent(requestUrl.pathname).replace(/^\/+/, '') || 'index.html';
    return await serveFile(res, root, rel);
  } catch {
    try { const data = await readFile(path.join(root,'404.html')); res.writeHead(404,{'Content-Type':'text/html; charset=utf-8'});res.end(data); }
    catch { res.writeHead(404); res.end('Not found'); }
  }
}).listen(port, host, ()=>{
  console.log(`SpaceTour Viewer : http://${host}:${port}/`);
  console.log(`SpaceTour Studio : http://${host}:${port}/studio/#/admin`);
  console.log('Studio is local-only and is never copied to GitHub Pages.');
});
